...
| `@legend` | bool (`off`) | on | Footer key decoding the dietary tags actually used (e.g. `v vegano · g sem glúten`), in the locale vocabulary's order. On by default since the pills have no hover tooltip on paper; set `off` to suppress |
...
### Appendix C — Template note (non-normative)
