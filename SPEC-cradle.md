...
The reference deployment uses **Embedded** for the QR-scale renderers (`menu`/`doorbell`/`arcr`/`contact`/`bio`, all inlined in the single-file bootloader) and a **separately-cached** variant for **`doc`** — the first renderer to use one. `doc`'s engine (vendored markdown-it + the doc renderer + CSS, ~57 KB gz) is too heavy to pay for on every cold-start, so it is NOT inlined: the bootloader lazy-loads it from `/cradle/doc/` (first-party, same-origin — the no-third-party-code rule holds) on `!doc1+` dispatch, and the service worker **runtime-caches** those assets on first fetch.

What is NOT conformant: opportunistic / on-demand caching that leaves an **embedded or pre-cached** renderer unavailable offline after its containing asset has been loaded once online. The baseline principle: once a user has loaded the cradle URL online, every *embedded/pre-cached* format is available offline forever after, until cache invalidation. A **separately-cached** renderer (the `doc` variant above) shifts the acquisition point: it is runtime-cached on **first use**, so it is offline-available after the first online render of that format rather than after the first bootloader load — a deliberate, documented trade so a user who never opens that format never downloads its weight. This is conformant *for renderers that opt into it*; the default offline guarantee (above) is unaffected for every embedded renderer.
